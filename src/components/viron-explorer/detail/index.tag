viron-explorer-detail.Explorer_Detail
  .Explorer_Detail__head
    .Explorer_Detail__close(onTap="{ handleCloseTap }")
      viron-icon-close
    .Explorer_Detail__delete(if="{ opts.isDeletable }" onTap="{ handleDeleteTap }") { i18n('compornents_viron_explorer_detail_title') }
  .Explorer_Detail__body
    .Explorer_Detail__inner(if="{ isReady }")
      ...
      .Explorer_Detail__infos
        ...
        .Explorer_Detail__info
          .Explorer_Detail__infoLabel { i18n('compornents_viron_explorer_detail_info') }
          .Explorer_Detail__infoValue(onTap="{ handlePathTap }") { selectedPath }
      viron-button(if="{ opts.isInsertable }" label="{ i18n('compornents_viron_explorer_detail_button') }" onSelect="{ handleInsertTap }")
