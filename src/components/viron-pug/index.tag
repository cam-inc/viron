viron-pug.Pug(class="{ 'Pug--disabled': opts.isdisabled, 'Pug--error': opts.haserror }")
  .Pug__tabs
    .Pug__tab(class="{ 'Pug__tab--selected': isEditorMode }" onTap="{ handleEditorTabTap }") { i18n('compornents_viron_pug_editor_label') }
    .Pug__tab(class="{ 'Pug__tab--selected': isPreviewMode }" onTap="{ handlePreviewTabTap }") { i18n('compornents_viron_pug_preview_label') }
  .Pug__body
