viron-parameters-form.Parameters_Form
  ...
  .Parameters_Form__body
    ...
    virtual(if="{ uiType === 'textarea' }")
      viron-textarea(val="{ opts.val }" isError="{ hasError }" onChange="{ handleTextareaChange }")
    virtual(if="{ uiType === 'numberinput' }")
      viron-numberinput(val="{ opts.val }" isError="{ hasError }" onChange="{ handleNumberinputChange }")
    virtual(if="{ uiType === 'checkbox' }")
      viron-checkbox(isChecked="{ opts.val }" isError="{ hasError }" label="{ title }" onChange="{ handleCheckboxChange }")
    virtual(if="{ uiType === 'select' }")
      viron-select(options="{ getSelectOptions() }" isError="{ hasError }" onChange="{ handleSelectChange }")
    virtual(if="{ uiType === 'uploader' }")
      viron-uploader(accept="*" isError="{ hasError }" onChange="{ handleUploaderChange }")
    virtual(if="{ uiType === 'html' }")
      viron-html(val="{ opts.val }" isError="{ hasError }" onChange="{ handleHtmlChange }")
    virtual(if="{ uiType === 'pug' }")
      viron-pug(val="{ opts.val }" isError="{ hasError }" onChange="{ handlePugChange }")
    virtual(if="{ uiType === 'autocomplete' }")
      viron-autocomplete(val="{ opts.val }" isError="{ hasError }" config="{ autocompleteConfig }" onChange="{ handleAutocompleteChange }")
    virtual(if="{ uiType === 'wyswyg' }")
      viron-wyswyg(val="{ opts.val }" isError="{ hasError }" onChange="{ handleWyswygChange }")
