viron-parameters-parameter.Parameters_Parameter(class="{ 'Parameters_Parameter--' + spreadStyle  }")
  virtual(if="{ isFormMode }")
    viron-parameters-form(formObject="{ formObject }" identifier="{ parameterObject.name }" val="{ opts.val }" isDisabled="{ isFormDisabled }" onChange="{ handleValChange }")
  virtual(if="{ isPropertiesMode }")
    viron-parameters-properties(label="{ propertiesLabel }" identifier="{ parameterObject.name }" val="{ opts.val }" required="{ parameterObject.required }" propertiesObject="{ propertiesObject }" onChange="{ handleValChange }")
