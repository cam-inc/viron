viron-parameters-items.Parameters_Items(class="{ 'Parameters_Items--preview': opts.ispreview } Parameters_Items--{ opts.theme }")
  .Parameters_Items__head
    ...
    .Parameters_Items__openButton(if="{ !!opts.val && !!opts.val.length }" onTap="{ handleOpenAllButtonTap }") { i18n('compornents_viron_parameters_items_button') }
      ...
      .Parameters_Items__itemDetail
        .Parameters_Items__itemHead
          .Parameters_Items__closeButton(onTap="{ handleCloseButtonTap }")
            viron-icon-arrow-up
          .Parameters_Items__removeButton(if="{ !parent.opts.ispreview }" onTap="{ handleRemoveButtonTap }") { i18n('compornents_viron_parameters_items_button_remove') }
