viron-datepicker.Datepicker
  ...
  form.Datepicker__form(onSubmit="{ handleFormSubmit }")
    ...
    .Datepicker__calendar(if="{ opts.isshown }")
      .Datepicker__head
        .Datepicker__transition(onClick="{ handlePrevButtonClick }") &lt;
        .Datepicker__month { displayDate.year() }
          span { settingDateName.month[opts.language || 'ja'][displayDate.month()] }
        .Datepicker__transition(onClick="{ handleNextButtonClick }") &gt;
      .Datepicker__days
      ...
      .Datepicker__container
        virtual(each="{ cell in generateCalendar() }")
          viron-datepicker-cell(date="{ cell.date }" isCurrentMonth="{ cell.isCurrentMonth }" isToday="{ cell.isToday }" isSelected="{ cell.isSelected }" onClick="{ handleCellClick }")
  script.
